...
class SymbolTable():
    ...
    def add_var(self, id, ref_count, _type):
        raise Exception('TODO')
    ...
    def __init__(self, specifier, init_list):
        ...
        for id in self.init_list:
            if symtable.lookup_var(id.decl):
                compilation_err.append('Redeclaration')
            elif self.specifier.type_spec is 'void' and id.ref_count is 0:
                compilation_err.append('variable or field ‘b’ declared void')
            else:
                # FIXME: initialize the symbols
                # FIXME: check the array constant expression
                # FIXME: complete the struct init function
                symtable.add_var(id.decl, id.ref_count, self.specifier.type_spec)
